...
Build_Plate_Size = 238; //[180: Small (A1 Mini), 238: Standard (X1/P1/A1), 350: Large (350mm)]
...
magnet_tab_min_bottom_vertical_height = Magnet_Hole_Depth + max(0, Minimum_Under_Magnet_Thickness - Solid_Base_Thickness);
magnet_tab_wall_thickness = 1.2;

filament_lock_top_thickness = 0.25;
filament_lock_min_bottom_thickness = 0.4;
filament_lock_min_bottom_vertical_height = Filament_Lock_Hole_Diameter + filament_lock_top_thickness + max(0, filament_lock_min_bottom_thickness - Solid_Base_Thickness);

b_top_chamfer_height = 1.9;
b_center_height = 1.8;
b_bottom_chamfer_height = 0.8;
b_bottom_vertical_height = Magnets ? max(Interlock_Type == 3 ? m3_min_bottom_vertical_height : filament_lock_min_bottom_vertical_height, magnet_tab_min_bottom_vertical_height) : (Interlock_Type == 3 ? m3_min_bottom_vertical_height : 0);
...
b_join_hole_center_down_offset = Interlock_Type == 3 ? m3_min_bottom_vertical_height / 2 : (filament_lock_top_thickness + Filament_Lock_Hole_Diameter / 2);
b_join_hole_center_height = b_total_height - b_top_chamfer_height - b_center_height - b_bottom_chamfer_height - b_join_hole_center_down_offset;
...
module uncut_baseplate(units_x, units_y, r_fl, r_bl, r_br, r_fr, m_l, m_b, m_r, m_f) {
  ...
  difference() {
    ...
    if (Margin_Skeletonization > 0) {
      margin_cutting_tools(units_x, units_y, m_l + margin_hole_margin_adjust, m_b + margin_hole_margin_adjust, m_r + margin_hole_margin_adjust, m_f + margin_hole_margin_adjust);
    }
  }
}
...
function GetCutOffsetForward(prev_offset, margin_start, margin_end, axis_unit_length) =
  let (prev_carry_over = prev_offset == 0 ? margin_start : left_tab_extent, remaining = prev_carry_over + (axis_unit_length - prev_offset) * Base_Unit_Dimension + margin_end, next_offset = prev_offset + floor((selected_plate_size - left_tab_extent - right_tab_extent - (prev_offset == 0 ? margin_start : 0)) / Base_Unit_Dimension), next_remaining_approx = (axis_unit_length - next_offset) * Base_Unit_Dimension + margin_end)

    remaining <= selected_plate_size ? -1 : (next_remaining_approx < Base_Unit_Dimension + grips_min_margin_for_full_tab + 0.001) ? next_offset - 1 : next_offset;

function GetAltEndOffset(margin_end, axis_unit_length) =
  let (space_for_full_units = selected_plate_size - margin_end - left_tab_extent - (axis_unit_length % 1) * Base_Unit_Dimension, full_units = floor(space_for_full_units / Base_Unit_Dimension), offset = floor(axis_unit_length) - full_units)
    offset;

function GetUnitsPerInnerSection(axis_unit_length) =
  let (space_for_full_units = selected_plate_size - left_tab_extent - right_tab_extent)
    floor(space_for_full_units / Base_Unit_Dimension);

function GetAltStartOffset(margin_start, margin_end, axis_unit_length) =
  let (end_offset = GetAltEndOffset(margin_end, axis_unit_length), units_per_inner_section = GetUnitsPerInnerSection(axis_unit_length), initial_offset = end_offset % units_per_inner_section, adjusted_offset = (initial_offset == 0) ? (Base_Unit_Dimension * units_per_inner_section + right_tab_extent + margin_start <= selected_plate_size) ? units_per_inner_section : 1 : initial_offset, final_offset = (adjusted_offset == 1 && margin_start < (grips_min_margin_for_full_tab + 0.001)) ? 2 : adjusted_offset)
    final_offset;
...
function recurse_plates_x(x_depth = 0, start_offset = 0) =
  let (end_offset = GetCutOffsetForward(start_offset, margin_left, margin_right, units_wide))
    ...
    concat(recurse_plates_y(x_depth, start_offset, end_offset), recurse_plates_x(x_depth + 1, end_offset));

function recurse_plates_y(x_depth, x_start_offset, x_end_offset, y_depth = 0, y_start_offset = 0) =
  let (alt_cuts = x_depth % 2 != 0, standard_offset = GetCutOffsetForward(y_start_offset, margin_front, margin_back, units_deep), y_end_offset = alt_cuts && y_start_offset == 0 && standard_offset >= 0 ? GetAltStartOffset(margin_front, margin_back, units_deep) : standard_offset)
...
function plate_centering_translation(plate) =
  let (x_start = plate[2], x_end = plate[3], y_start = plate[4], y_end = plate[5], left_amount = x_start == 0 ? margin_left : left_tab_extent, right_amount = (x_end - x_start) * Base_Unit_Dimension + (x_end == units_wide ? margin_right : right_tab_extent), front_amount = y_start == 0 ? margin_front : left_tab_extent, back_amount = (y_end - y_start) * Base_Unit_Dimension + (y_end == units_deep ? margin_back : right_tab_extent), filament_cutter_avoidance_x = do_x1c_adjustment ? max(18 - (256 - (left_amount + right_amount)) / 2, 0) : 0, filament_cutter_avoidance_y = do_x1c_adjustment ? max(18 - (256 - (front_amount + back_amount)) / 2, 0) : 0, )
    [(left_amount - right_amount) / 2 + filament_cutter_avoidance_x, (front_amount - back_amount) / 2 + filament_cutter_avoidance_y, 0];

function overall_centering_translation(plate) =
  let (x_depth = plate[0], y_depth = plate[1], x_start_offset = plate[2], y_start_offset = plate[4], assembled_center = [-Base_Unit_Dimension * units_wide / 2, -Base_Unit_Dimension * units_deep / 2, 0], exiting_plate_center = plate_centering_translation(plate), plate_offset = [x_start_offset * Base_Unit_Dimension + part_spacing * x_depth, y_start_offset * Base_Unit_Dimension + part_spacing * y_depth, 0], combined = assembled_center + plate_offset - exiting_plate_center, )
    [Mirror_Horizontally ? -combined.x : combined.x, Mirror_Vertically ? -combined.y : combined.y, combined.z];
...
module sub_baseplate_from_list(plate) {
  translate(plate_centering_translation(plate))
    sub_baseplate(plate[0], plate[1], // x_depth, y_depth
...
// For MakerWorld multi-plate support.
// Delete when not needed: MakerWorld will see the plate names even if commented out.
function size_check() =
  len(plates) <= 36;
assert(size_check(), "Too many build plates for 3MF format (max 36). For very large baseplates, try the STL version of this customizer.");

module mw_plate_1() {
  mirrored_base_plate(plates[0]);
}
module mw_plate_2() {
  if (len(plates) >= 2)
    mirrored_base_plate(plates[1]);
}
module mw_plate_3() {
  if (len(plates) >= 3)
    mirrored_base_plate(plates[2]);
}
module mw_plate_4() {
  if (len(plates) >= 4)
    mirrored_base_plate(plates[3]);
}
module mw_plate_5() {
  if (len(plates) >= 5)
    mirrored_base_plate(plates[4]);
}
module mw_plate_6() {
  if (len(plates) >= 6)
    mirrored_base_plate(plates[5]);
}
module mw_plate_7() {
  if (len(plates) >= 7)
    mirrored_base_plate(plates[6]);
}
module mw_plate_8() {
  if (len(plates) >= 8)
    mirrored_base_plate(plates[7]);
}
module mw_plate_9() {
  if (len(plates) >= 9)
    mirrored_base_plate(plates[8]);
}
module mw_plate_10() {
  if (len(plates) >= 10)
    mirrored_base_plate(plates[9]);
}
module mw_plate_11() {
  if (len(plates) >= 11)
    mirrored_base_plate(plates[10]);
}
module mw_plate_12() {
  if (len(plates) >= 12)
    mirrored_base_plate(plates[11]);
}
module mw_plate_13() {
  if (len(plates) >= 13)
    mirrored_base_plate(plates[12]);
}
module mw_plate_14() {
  if (len(plates) >= 14)
    mirrored_base_plate(plates[13]);
}
module mw_plate_15() {
  if (len(plates) >= 15)
    mirrored_base_plate(plates[14]);
}
module mw_plate_16() {
  if (len(plates) >= 16)
    mirrored_base_plate(plates[15]);
}
module mw_plate_17() {
  if (len(plates) >= 17)
    mirrored_base_plate(plates[16]);
}
module mw_plate_18() {
  if (len(plates) >= 18)
    mirrored_base_plate(plates[17]);
}
module mw_plate_19() {
  if (len(plates) >= 19)
    mirrored_base_plate(plates[18]);
}
module mw_plate_20() {
  if (len(plates) >= 20)
    mirrored_base_plate(plates[19]);
}
module mw_plate_21() {
  if (len(plates) >= 21)
    mirrored_base_plate(plates[20]);
}
module mw_plate_22() {
  if (len(plates) >= 22)
    mirrored_base_plate(plates[21]);
}
module mw_plate_23() {
  if (len(plates) >= 23)
    mirrored_base_plate(plates[22]);
}
module mw_plate_24() {
  if (len(plates) >= 24)
    mirrored_base_plate(plates[23]);
}
module mw_plate_25() {
  if (len(plates) >= 25)
    mirrored_base_plate(plates[24]);
}
module mw_plate_26() {
  if (len(plates) >= 26)
    mirrored_base_plate(plates[25]);
}
module mw_plate_27() {
  if (len(plates) >= 27)
    mirrored_base_plate(plates[26]);
}
module mw_plate_28() {
  if (len(plates) >= 28)
    mirrored_base_plate(plates[27]);
}
module mw_plate_29() {
  if (len(plates) >= 29)
    mirrored_base_plate(plates[28]);
}
module mw_plate_30() {
  if (len(plates) >= 30)
    mirrored_base_plate(plates[29]);
}
module mw_plate_31() {
  if (len(plates) >= 31)
    mirrored_base_plate(plates[30]);
}
module mw_plate_32() {
  if (len(plates) >= 32)
    mirrored_base_plate(plates[31]);
}
module mw_plate_33() {
  if (len(plates) >= 33)
    mirrored_base_plate(plates[32]);
}
module mw_plate_34() {
  if (len(plates) >= 34)
    mirrored_base_plate(plates[33]);
}
module mw_plate_35() {
  if (len(plates) >= 35)
    mirrored_base_plate(plates[34]);
}
module mw_plate_36() {
  if (len(plates) >= 36)
    mirrored_base_plate(plates[35]);
}

module mw_assembly_view() {
  translate(overall_centering_translation(plates[0]))
    mw_plate_1();
  if (len(plates) >= 2)
    translate(overall_centering_translation(plates[1]))
      mw_plate_2();
  if (len(plates) >= 3)
    translate(overall_centering_translation(plates[2]))
      mw_plate_3();
  if (len(plates) >= 4)
    translate(overall_centering_translation(plates[3]))
      mw_plate_4();
  if (len(plates) >= 5)
    translate(overall_centering_translation(plates[4]))
      mw_plate_5();
  if (len(plates) >= 6)
    translate(overall_centering_translation(plates[5]))
      mw_plate_6();
  if (len(plates) >= 7)
    translate(overall_centering_translation(plates[6]))
      mw_plate_7();
  if (len(plates) >= 8)
    translate(overall_centering_translation(plates[7]))
      mw_plate_8();
  if (len(plates) >= 9)
    translate(overall_centering_translation(plates[8]))
      mw_plate_9();
  if (len(plates) >= 10)
    translate(overall_centering_translation(plates[9]))
      mw_plate_10();
  if (len(plates) >= 11)
    translate(overall_centering_translation(plates[10]))
      mw_plate_11();
  if (len(plates) >= 12)
    translate(overall_centering_translation(plates[11]))
      mw_plate_12();
  if (len(plates) >= 13)
    translate(overall_centering_translation(plates[12]))
      mw_plate_13();
  if (len(plates) >= 14)
    translate(overall_centering_translation(plates[13]))
      mw_plate_14();
  if (len(plates) >= 15)
    translate(overall_centering_translation(plates[14]))
      mw_plate_15();
  if (len(plates) >= 16)
    translate(overall_centering_translation(plates[15]))
      mw_plate_16();
  if (len(plates) >= 17)
    translate(overall_centering_translation(plates[16]))
      mw_plate_17();
  if (len(plates) >= 18)
    translate(overall_centering_translation(plates[17]))
      mw_plate_18();
  if (len(plates) >= 19)
    translate(overall_centering_translation(plates[18]))
      mw_plate_19();
  if (len(plates) >= 20)
    translate(overall_centering_translation(plates[19]))
      mw_plate_20();
  if (len(plates) >= 21)
    translate(overall_centering_translation(plates[20]))
      mw_plate_21();
  if (len(plates) >= 22)
    translate(overall_centering_translation(plates[21]))
      mw_plate_22();
  if (len(plates) >= 23)
    translate(overall_centering_translation(plates[22]))
      mw_plate_23();
  if (len(plates) >= 24)
    translate(overall_centering_translation(plates[23]))
      mw_plate_24();
  if (len(plates) >= 25)
    translate(overall_centering_translation(plates[24]))
      mw_plate_25();
  if (len(plates) >= 26)
    translate(overall_centering_translation(plates[25]))
      mw_plate_26();
  if (len(plates) >= 27)
    translate(overall_centering_translation(plates[26]))
      mw_plate_27();
  if (len(plates) >= 28)
    translate(overall_centering_translation(plates[27]))
      mw_plate_28();
  if (len(plates) >= 29)
    translate(overall_centering_translation(plates[28]))
      mw_plate_29();
  if (len(plates) >= 30)
    translate(overall_centering_translation(plates[29]))
      mw_plate_30();
  if (len(plates) >= 31)
    translate(overall_centering_translation(plates[30]))
      mw_plate_31();
  if (len(plates) >= 32)
    translate(overall_centering_translation(plates[31]))
      mw_plate_32();
  if (len(plates) >= 33)
    translate(overall_centering_translation(plates[32]))
      mw_plate_33();
  if (len(plates) >= 34)
    translate(overall_centering_translation(plates[33]))
      mw_plate_34();
  if (len(plates) >= 35)
    translate(overall_centering_translation(plates[34]))
      mw_plate_35();
  if (len(plates) >= 36)
    translate(overall_centering_translation(plates[35]))
      mw_plate_36();
}
